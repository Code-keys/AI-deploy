    ...
    int current_det_fps = -1, int current_cap_fps = -1)
{
    ...
    for (auto &i : result_vec) {
        ...
        /*if (obj_names.size() > i.obj_id) {
            ...
            putText(mat_img, std::to_string(i.obj_id), cv::Point2f(i.x, i.y - 16), cv::FONT_HERSHEY_COMPLEX_SMALL, 1.2, cv::Scalar(0, 0, 0), 2);
...
{
    std::cout << "input class names ";
    std::string  names_file;
    // std::cin >> names_file ;
    if(names_file.size() == 0)   names_file = "coco.names";

    std::cout << "input cfg_file cfg_file ";
    std::string  cfg_file;
    std::cin >> cfg_file;
    if(cfg_file.size() == 0)   cfg_file = "/media/cx/Projects/Yolov3Train/Model_cp/yolov3_tiny_4L_608/yolov3-voc-test.cfg";
    
    std::cout << "input weights_file weights_file ";
    std::string  weights_file;
    std::cin >> weights_file;
    if(weights_file.size() == 0)   weights_file ="/media/cx/Projects/Yolov3Train/Model_cp/yolov3_tiny_4L_608/yolov3-voc-test.weights";
    ...
    float const thresh = (argc > 5) ? std::stof(argv[5]) : 0.05;
        ...
        try {
...

            {
                ...
                if (!file.is_open()) std::cout << "File not found! \n";
                    ...
                    vector<string> filenamesplit=split(line,'/');
                    ...
                    imwrite(outputfile, mat_img);
                        ...
                        // vector<string> filenamesplit=split(line,'/');
                        // string endname=filenamesplit[filenamesplit.size()-1];
                        // endname.replace(endname.end()-4,endname.end(),"_pre.jpg");
                        // std::string outputfile="detect_result/"+endname;
                        // imwrite(outputfile, mat_img);
                        // //draw_boxes(mat_img, result_vec, obj_names);
                        // cv::imwrite(line, mat_img);
