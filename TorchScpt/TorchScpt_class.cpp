    ...
    device_(torch::kCPU), GCpuId_(gpuid),class_names( LoadNames(class_path) )
{
    ...
    device_ =  GCpuId_ == -1 ? torch::kCPU : torch::kCUDA ; 
    ...
    // infer once to CUDAmalloc 
    auto temp_img = cv::Mat::zeros(640, 640, CV_8UC3);
    Run(temp_img, 1.0f, 1.0f);
}
...
Detector::Run(const cv::Mat& img, float conf_threshold, float iou_threshold) {
    ...
    std::vector<float> pad_info = LetterboxImage(img_input, img_input, cv::Size(INPUT_W, INPUT_H));
    ...
    // std::cout << " pre-process takes : " << duration.count() << " ms ";
    ...
    // It should be known that it takes longer time at first time
    // std::cout << "inference takes : " << duration.count() << " ms "  ;
...
cv::Mat&
Detector::draw_result(  cv::Mat& img,
...
cv::Mat&
Detector::pic2pic(cv::Mat& img,
                   float conf_threshold, float iou_threshold ){
   return draw_result(img,Run(img,conf_threshold,conf_threshold));
};
                   
cv::Mat&
Detector::path2pic(   const std::string img_path,
                      float conf_threshold, float iou_threshold ){
...
std::vector<float>
Detector::LetterboxImage(const cv::Mat& src, cv::Mat& dst, const cv::Size& out_size) {
...
std::vector<std::vector<Detection>>
Detector::PostProcessing(   const torch::Tensor& detections,
...
void Detector::ChangeNames(const std::vector<std::string>& names  ){ 
    class_names = names;
}
...
float Detector::predict_cv(cv::Mat& img, float conf_thres,float iou_thres  ){
