...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt ) {
    ...
    auto pool43 = network->addPoolingNd(*l42->getOutput(0), PoolingType::kMAX, DimsHW{5, 5});
    pool43->setPaddingNd(DimsHW{2, 2});
    pool43->setStrideNd(DimsHW{1, 1});
    auto l44 = l42; 
    auto pool45 = network->addPoolingNd(*l42->getOutput(0), PoolingType::kMAX, DimsHW{9, 9});
    pool45->setPaddingNd(DimsHW{4, 4});
    pool45->setStrideNd(DimsHW{1, 1}); 
    auto l46 = l42; 
    auto pool47 = network->addPoolingNd(*l42->getOutput(0), PoolingType::kMAX, DimsHW{13, 13});
    pool47->setPaddingNd(DimsHW{6, 6});
    pool47->setStrideNd(DimsHW{1, 1}); 
    ...
    IDeconvolutionLayer* deconv58 = network->addDeconvolutionNd(*l57->getOutput(0), 256 * YOLOv5_cfg_ratio , DimsHW{2, 2}, deconvwts58, emptywts);
    assert(deconv58);
    deconv58->setStrideNd(DimsHW{2, 2});
    ...
    IDeconvolutionLayer* deconv68 = network->addDeconvolutionNd(*l67->getOutput(0) , 128 * YOLOv5_cfg_ratio, DimsHW{2, 2}, deconvwts68, emptywts);
    assert(deconv68);
    deconv68->setStrideNd(DimsHW{2, 2});
    ...
    auto convl77 = network->addConvolutionNd(*l76->getOutput(0), 3 * (Yolo::CLASS_NUM + 5), DimsHW{1, 1}, weightMap["module_list.77.Conv2d.weight"], weightMap["module_list.77.Conv2d.bias"]);
    assert(convl77); 
    // 78 is yolo layer
    ...
    IConvolutionLayer* convl89 = network->addConvolutionNd(*l88->getOutput(0), 3 * (Yolo::CLASS_NUM + 5), DimsHW{1, 1}, weightMap["module_list.89.Conv2d.weight"], weightMap["module_list.89.Conv2d.bias"]);
    assert(convl89);
    ...
    IConvolutionLayer* convl01 = network->addConvolutionNd(*l100->getOutput(0), 3 * (Yolo::CLASS_NUM + 5), DimsHW{1, 1}, weightMap["module_list.101.Conv2d.weight"], weightMap["module_list.101.Conv2d.bias"]);
    assert(convl01);
    // 102 is yolo layer
    ...
    ITensor* inputTensors_yolo[] = {convl77->getOutput(0), convl89->getOutput(0), convl01->getOutput(0)};
