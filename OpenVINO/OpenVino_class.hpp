...
class VINODetector {
    cv::Mat static_resize(cv::Mat& img) {
        float r = std::min(INPUT_W / (img.cols*1.0), INPUT_H / (img.rows*1.0));   // r = std::min(r, 1.0f);
...
static void qsort_descent_inplace(std::vector<Object>& objects)
{
    ...
    qsort_descent_inplace(objects, 0, objects.size() - 1);  // 迭代求解
}
    ...
    static void decode_outputs(const float* prob, std::vector<Object>& objects, float scale, const int img_w, const int img_h, float BBOX_CONF_THRESH,float NMS_THRESH) {
    ...
    static void draw_objects(const cv::Mat& bgr, const std::vector<Object>& objects)
    {
        ...
        // cv::imwrite("_demo.jpg" , image); // fprintf(stderr, "save vis file\n");
        /* cv::imshow("image", image); */ /* cv::waitKey(0); */
    }
public:
    VINODetector(std::string p,int w, int h, std::string dev): input_model(p), INPUT_W(w), INPUT_H(h), device_name(dev){
        ...
        /* Mark input as resizable by setting of a resize algorithm.    * In this case we will be able to set an input blob of any shape to an
         * infer request. Resize and layout conversions are executed automatically   * during inference */
    ...
    float predict_cv(cv::Mat& img, float conf_thresh=0.25, float nms_thresh=0.45){ 
        ...
        // --------------------------- Step 7. Do inference  
        infer_request.Infer();  /* Running the request synchronously */
        ...
        decode_outputs(net_pred, objects, scale, img_w, img_h, conf_thresh, nms_thresh);
