...
TF_FeatureTensor *TF_FeatureTensor::instance = NULL;

TF_FeatureTensor *TF_FeatureTensor::getInstance() {
	if(instance == NULL) {
		instance = new TF_FeatureTensor();
	}
	return instance;
}

TF_FeatureTensor::TF_FeatureTensor() {
	//prepare model:
	bool status = init();
	if(status == false)
	  {
...
bool TF_FeatureTensor::getRectsFeature(const cv::Mat& img, DETECTIONS& d) {
...
void TF_FeatureTensor::tobuffer(const std::vector<cv::Mat> &imgs, uint8 *buf) {
	int pos = 0;
